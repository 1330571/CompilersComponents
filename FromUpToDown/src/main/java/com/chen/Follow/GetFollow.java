        ...
        while (flag) {
            ...
            for (Language language : aLanguage.getList()) {
                ...
                for (int i = 0; i < language.getMatch().length - 1; i++) {
                    ...
                    if (IsNTerminal(a)) {
                        //遍历后面的每一个字符
                        for (int j = i + 1; j < language.getMatch().length; j++) {
                            First firstNext = findFirstBySymbol(language.getMatch()[j]);
                            //是非终结符 将下一个符号的first集元素-'$' 都加到当前非终结符的follow集合中
                            boolean IsEmptySymbol = false;
                            assert firstNext != null;
                ...
                for (j = language.getMatch().length - 1; j >= 0; j--) {
                    ...
                    if (findFirstBySymbol(a) != null && Objects.requireNonNull(findFirstBySymbol(a)).IsOfFirst('$')) {
                        continue;
                    }
